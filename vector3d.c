...
void mat4_mmul(Matrix4 * md, const Matrix4 * ms)
{
	for(char i=0; i<4; i++)
	{
		float	m0  = md->m[i + 0] * ms->m[ 0] + md->m[i + 4] * ms->m[ 1] + md->m[i + 8] * ms->m[ 2] + md->m[i + 12] * ms->m[ 3];
		float	m4  = md->m[i + 0] * ms->m[ 4] + md->m[i + 4] * ms->m[ 5] + md->m[i + 8] * ms->m[ 6] + md->m[i + 12] * ms->m[ 9];
		float	m8  = md->m[i + 0] * ms->m[ 8] + md->m[i + 4] * ms->m[ 9] + md->m[i + 8] * ms->m[10] + md->m[i + 12] * ms->m[11];
		float	m12 = md->m[i + 0] * ms->m[12] + md->m[i + 4] * ms->m[13] + md->m[i + 8] * ms->m[14] + md->m[i + 12] * ms->m[15];

		md->m[i + 0] = m0; md->m[i + 4] = m4; md->m[i + 8] = m8; md->m[i + 12] = m12;
	}
}
...
void mat4_rmmul(Matrix4 * md, const Matrix4 * ms)
{
	...
	{
		float	m0 = md->m[i + 0] * ms->m[0] + md->m[i + 1] * ms->m[4] + md->m[i + 2] * ms->m[ 8] + md->m[i + 3] * ms->m[12];
		float	m1 = md->m[i + 0] * ms->m[1] + md->m[i + 1] * ms->m[5] + md->m[i + 2] * ms->m[ 9] + md->m[i + 3] * ms->m[13];
		float	m2 = md->m[i + 0] * ms->m[2] + md->m[i + 1] * ms->m[6] + md->m[i + 2] * ms->m[10] + md->m[i + 3] * ms->m[14];
		float	m3 = md->m[i + 0] * ms->m[3] + md->m[i + 1] * ms->m[7] + md->m[i + 2] * ms->m[11] + md->m[i + 3] * ms->m[15];

		md->m[i + 0] = m0; md->m[i + 1] = m1; md->m[i + 2] = m2; md->m[i + 3] = m3;
	}
}
